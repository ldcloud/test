...
RUN yum clean all
RUN groupadd -r citrusgrp -g 1001 && useradd -s -u 1001 -r -g citrusgrp -m -d /opt/maven -s /usr/bin/bash -c "citrus" citrus && chmod 755 /opt/maven
...
WORKDIR /citrus
RUN echo "ALL ALL=NOPASSWD: /usr/bin/su - citrus" >> /etc/sudoers
