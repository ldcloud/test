...
RUN yum -y install which java-1.8.0-openjdk.x86_64 bind-utils nc telnet net-tools git apache-maven
RUN yum clean all
RUN groupadd -r citrusgrp -g 1001 && useradd -u 1001 -r -g citrusgrp -m -d /opt/maven -s /usr/bin/bash -c "citrus" citrus && chmod 755 /opt/maven
...
RUN mkdir /citrus && chmod 777 /citrus
WORKDIR /citrus
